...
if [[ -d "/var/tmp/cache/compiler" ]]; then
	...
	# The ccache ebuild has a bug where it will install links in /usr/lib/ccache/bin
	# to reflect the current setting of CHOST. But the current setting of CHOST may 
	# not reflect the current compiler available (remember, CHOST can be overridden 
	# in /etc/make.conf)

	# This causes problems with ebuilds (such as ncurses) who may find an 
	# "i686-pc-linux-gnu-gcc" symlink in /usr/lib/ccache/bin and assume that an 
	# "i686-pc-linux-gnu-gcc" compiler is actually installed, when we really have an 
	# i486-pc-linux-gnu-gcc compiler installed. For some reason, ncurses ends up 
	# looking for the compiler in /usr/bin and it fails - no compiler found.

	# It's a weird problem but the next few ccache-config lines takes care of it by
	# removing bogus ccache symlinks and installing valid ones that reflect the 
	# compiler that is actually installed on the system - so if ncurses sees an 
	# "i686-pc-linux-gnu-gcc" in /usr/lib/ccache/bin, it looks for (and finds) a 
	# real i686-pc-linux-gnu-gcc installed in /usr/bin.

	# I am including these detailed notes so that people are aware of the issue and
	# so we can look for a more elegant solution to this problem in the future. This
	# problem crops up when you are using an i486-pc-linux-gnu CHOST stage3 to create an
	# i686-pc-linux-gnu CHOST stage1. It will probably crop up whenever the CHOST gets
	# changed. For now, it's fixed :)

	if [[ -e "/usr/bin/ccache-config" ]]; then
		for x in i386 i486 i586 i686 x86_64; do
		...
		if [[ "$?" -eq 0 ]]; then
			gccchost=`gcc-config -S ${gccprofile} | cut -f 1 -d " "`
			echo "[Metro] Setting ccache links to: ${gccchost}"
			ccache-config --install-links ${gccchost}
		else
			echo "[Metro] There was an error using gcc-config. Ccache not enabled."
			unset CCACHE_DIR
			export FEATURES="${FEATURES} -ccache"
		fi
	fi
...
# Sets things necessary for us to create and use binary packages
if [[ -e "/var/tmp/cache/package" ]]; then
...
# The quotes below prevent variable expansion for anything inside make.conf
...
if [[ "${pf}" == "new" ]]; then
	...
	mixins="" && mixins=$[profile/mix-ins:zap]
	...
	# This empty echo needs to be here because if we are using new profiles,
	# the below two lines will be zapped/removed and if no echo statement is here,
	# then this will just be an empty else, causing a bash error.
	echo ""
	ln -sf ../usr/portage/profiles/$[portage/profile:zap] /etc/make.profile || exit 1
	echo "[Metro] Set portage profile to $[portage/profile:zap]."
fi

# Mount 'devtmpfs' so we don't get the following message:
# openpty failed: 'out of pty devices'
mount -t devtmpfs none /dev

# Read the news to hide annoying 'new news' messsages
...
postrun: [
#!/bin/bash
if [[ "$[target]" != "stage1" ]] && [[ -e "/usr/bin/ccache" ]]; then
...
# Unmount 'devtmpfs' since we are done using it
umount /dev
]
...
if [[ "${ROOT}" == "/" ]]; then
	# Clean our chroot; Remove our tweaked configuration files, and restore originals
	for f in /etc/profile.env /etc/csh.env /etc/env.d/99zzmetro; do
		rm -f ${f} || exit 1
	done
...
for x in passwd group shadow; do
	rm -f ${ROOT}/etc/${x}-
...
if [[ ! -e "${ROOT}/etc/portage/make.conf.example" ]] && [[ ! -L "${ROOT}/etc/portage/make.conf.example" ]]; then
	if [[ -e "${ROOT}/usr/share/portage/config/make.conf.example" ]]; then
	...
	for file in files:
		...
		if os.path.exists(myfile):
			...
			if myperms not in perms:
				print("[Metro] ERROR: file %s does not have proper perms: %s (should be one of %s)" % ( myfile, myperms, perms ))
				abort = True

			if myuid != uid:
				print("[Metro] ERROR: file %s does not have uid of %i" % ( myfile, uid ))
				abort = True

			if mygid != gid:
				print("[Metro] ERROR: file %s does not have gid of %i" % ( myfile, gid ))
				abort = True
